...
export const DEMO_DATABASE_METADATA = {
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
JOIN \`former-prod.demo_thelook_ecommerce.inventory_items\` AS ii ON oi.inventory_item_id = ii.id
JOIN \`former-prod.demo_thelook_ecommerce.distribution_centers\` AS dc ON ii.product_distribution_center_id = dc.id
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.users\` u
ON oi.user_id = u.id
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o ON oi.order_id = o.order_id
WHERE oi.status NOT IN ('Cancelled', 'Returned') 
...
FROM \`former-prod.demo_thelook_ecommerce.users\` -- From the users table
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
 LEFT JOIN \`former-prod.demo_thelook_ecommerce.products\` AS p ON oi.product_id = p.id
WHERE oi.status NOT IN ('Cancelled','Returned')
GROUP BY 1
ORDER BY 3 DESC`
  },
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` as oi 
JOIN \`former-prod.demo_thelook_ecommerce.products\` as p 
...
ORDER BY 1 DESC`
  ...
  AVG(
    (
      ...
      FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oitemp 
      WHERE oitemp.order_id = o.order_id
    )
  ) AS avg_sku_sold
FROM \`former-prod.demo_thelook_ecommerce.events\` AS e
JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u 
  ON e.user_id = u.id
JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o 
  ON u.id = o.user_id
JOIN \`former-prod.demo_thelook_ecommerce.order_items\` AS oi 
  ON o.order_id = oi.order_id
JOIN \`former-prod.demo_thelook_ecommerce.inventory_items\` AS ii 
...
FROM \`former-prod.demo_thelook_ecommerce.products\` as p 
JOIN \`former-prod.demo_thelook_ecommerce.order_items\` as oi
...
ORDER BY num_of_items_sold DESC`
  ...
  FROM \`former-prod.demo_thelook_ecommerce.orders\` AS o
  JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u 
    ON u.id = o.user_id
  GROUP BY u.id
), 
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o 
...
ORDER BY 1 DESC`
  ...
  id,
  email
FROM \`former-prod.demo_thelook_ecommerce.users\` 
...
FROM (
  ...
  SELECT DISTINCT
    ...
    u.country
  FROM
    \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
  JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u ON oi.user_id = u.id
  WHERE
...
INNER JOIN (
  ...
  FROM \`former-prod.demo_thelook_ecommerce.users\` AS u
  INNER JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o ON u.id = o.user_id
  GROUP BY u.id
...
ORDER BY sale_date`
  ...
  FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
  INNER JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u 
  ON oi.user_id = u.id
  WHERE oi.status NOT IN ('Cancelled','Returned')
...
FROM (
  ...
  FROM \`former-prod.demo_thelook_ecommerce.order_items\` oi
  JOIN \`former-prod.demo_thelook_ecommerce.products\` p
  ON oi.product_id = p.id
...
JOIN \`former-prod.demo_thelook_ecommerce.order_items\` o
ON swim_orders.order_id = o.order_id
JOIN \`former-prod.demo_thelook_ecommerce.products\` p1
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.users\` u ON oi.user_id = u.id
WHERE oi.status NOT IN ('Cancelled','Returned')
GROUP BY 1, 2
ORDER BY 3 DESC
LIMIT 10;`
  ...
  FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
  JOIN \`former-prod.demo_thelook_ecommerce.products\` AS p ON oi.product_id = p.id
  WHERE oi.status NOT IN ('Cancelled', 'Returned')
...
ORDER BY user_group DESC, total_category_spend DESC`
 ...
 COUNT(oi.id) AS quantity
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.products\` AS p
ON oi.product_id = p.id
WHERE oi.status NOT IN ('Cancelled','Returned')
GROUP BY 1
ORDER BY 3 DESC`
  ...
  FROM \`former-prod.demo_thelook_ecommerce.products\` as p
  INNER JOIN \`former-prod.demo_thelook_ecommerce.order_items\` as oi
  ON p.id = oi.product_id
  WHERE EXTRACT(YEAR FROM DATE(oi.created_at)) = EXTRACT(YEAR FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 YEAR))
  GROUP BY 1, 2, 3
)
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` oi
    LEFT JOIN \`former-prod.demo_thelook_ecommerce.products\` p
    ON oi.product_id = p.id
GROUP BY 1
ORDER BY 2 DESC;`
  },
...
LEFT JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o 
    ON oi.order_id = o.order_id
LEFT JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u 
    ON o.user_id = u.id
WHERE oi.status NOT IN ('Cancelled','Returned')
 ...
 gender,
FROM
 \`former-prod.demo_thelook_ecommerce.orders\` /* Specifying the table from which to retrieve the orders */
ORDER BY 
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o ON oi.order_id = o.order_id
LEFT JOIN \`former-prod.demo_thelook_ecommerce.users\` AS u ON o.user_id = u.id
WHERE oi.status NOT IN ('Cancelled','Returned')
GROUP BY 1, 2
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` oi
LEFT JOIN \`former-prod.demo_thelook_ecommerce.orders\` o
ON oi.order_id = o.order_id
WHERE oi.status NOT IN ('Cancelled','Returned')
GROUP BY 1
ORDER BY 2`
...
FROM \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
JOIN \`former-prod.demo_thelook_ecommerce.orders\` AS o ON oi.order_id = o.order_id
JOIN \`former-prod.demo_thelook_ecommerce.inventory_items\` AS ii ON oi.inventory_item_id = ii.id
WHERE oi.status NOT IN ('Cancelled', 'Returned')
...
GROUP BY oi.order_id, o.user_id`
  },
  {
    ...
    description: "Finds the top 10 best-selling products over the last 6 months, grouping by product ID and name and ordering by the number of items sold in descending order. Utilizes the 'products' and 'order_items' tables from the 'former-prod.demo_thelook_ecommerce' dataset.",
  ...
  SELECT 
    ...
    COUNT(oi.id) AS num_of_items_sold
  FROM 
    \`former-prod.demo_thelook_ecommerce.products\` AS p
  INNER JOIN 
    \`former-prod.demo_thelook_ecommerce.order_items\` AS oi
  ON 
    p.id = oi.product_id
...
FROM \`former-prod.demo_thelook_ecommerce.inventory_items\` AS ii
LEFT JOIN (
  SELECT DISTINCT inventory_item_id
  FROM \`former-prod.demo_thelook_ecommerce.order_items\`
